...
func RunEvaluation(ctx context.Context, c *Client, r *Report) error {
	...
	for _, w := range workloads {
		for i := 1; i <= r.FreshRuns; i++ {
			if err := freshRun(ctx, c, r, w, i); err != nil { errs = append(errs, err) }
		}
		if err := reuseRun(ctx, c, r, w); err != nil { errs = append(errs, err) }
	}
	r.Summarize()
...
func reuseRun(ctx context.Context, c *Client, r *Report, w Workload) error {
	...
	for i := 1; i <= r.ReuseRuns; i++ {
		if err := measuredExecute(ctx, c, r, w.Name, "reuse", i, s.ID, w.Command); err != nil { return err }
	}
	return nil
}
...
func measuredCreate(ctx context.Context, c *Client, r *Report, workload, mode string, run int) (Sandbox, error) {
	var s Sandbox
	ms, err := timed(func() error { var e error; s, e = c.Create(ctx); return e })
...
func measuredExecute(ctx context.Context, c *Client, r *Report, workload, mode string, run int, id, command string) error {
	...
	ms, err := timed(func() error { var e error; response, e = c.Execute(ctx, id, command); return e })
	if err == nil && response.ExitCode != 0 { err = fmt.Errorf("exit code %d: %s", response.ExitCode, strings.TrimSpace(response.Stderr)) }
	r.add(Sample{Workload: workload, Mode: mode, Run: run, Operation: "execute", DurationMS: ms, SandboxID: id, Status: status(err), Error: errText(err)})
	return err
}
...
func (r *Report) add(s Sample) {
	...
	if s.Status != "ok" { r.Failures["operation:"+s.Operation]++ }
}

func (r *Report) Summarize() {
	groups := map[string][]float64{}
	for _, s := range r.Samples { if s.Status == "ok" { groups[s.Workload+"/"+s.Mode+"/"+s.Operation] = append(groups[s.Workload+"/"+s.Mode+"/"+s.Operation], s.DurationMS) } }
	for key, values := range groups { r.Summaries[key] = summarize(values) }
}
...
func errText(err error) string { if err != nil { return err.Error() }; return "" }
